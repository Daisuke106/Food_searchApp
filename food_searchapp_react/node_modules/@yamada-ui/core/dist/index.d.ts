...
import { StringLiteral, Dict, Union, Merge } from '@yamada-ui/utils';
import * as CSS from 'csstype';
export { CSS };
import { PortalProps } from '@yamada-ui/portal';
import { Variants, MotionConfigProps } from 'framer-motion';
...
type AlertStatuses = Partial<Record<AlertStatus, {
    icon?: FC;
    colorScheme?: Theme["colorSchemes"];
}>>;
type AlertStatus = "info" | "success" | "warning" | "error" | "loading";
type NoticeComponentProps = NoticeConfigOptions & {
    onClose: () => void;
};
type NoticeConfigOptions = ThemeProps<"Alert"> & {
    /**
...
type SnackDirection = "top" | "bottom";
type SnackComponentProps = SnackConfigOptions & {
    index: number;
    onClose: () => void;
};
type SnackConfigOptions = ThemeProps<"Alert"> & {
    /**
...
type LayerStyles = Record<string, CSSUIObject>;
type TextStyles = Record<string, CSSUIObject>;
type ThemeTokens = {
    [key: string | number]: string | number | [string | number, string | number] | ThemeTokens;
};
type ThemeAnimationTokens<T extends AnimationStyle | string = AnimationStyle> = {
    [key: string | number]: T | T[] | ThemeAnimationTokens<T>;
};
type ThemeTransitionTokens = {
    property?: ThemeTokens;
    duration?: ThemeTokens;
    easing?: ThemeTokens;
};
type ThemeBreakpointTokens = {
    [key: string | number]: string | number;
};
type ThemeSemantics = Omit<BaseTheme, "styles" | "components" | "semantics" | "themeSchemes" | "breakpoints" | "animations"> & {
    colorSchemes?: Partial<Record<string, Theme["colorSchemes"]>>;
    animations?: ThemeAnimationTokens<AnimationStyle | string>;
};
type ThemeSchemes = Partial<Record<string, Omit<BaseTheme, "styles" | "components" | "themeSchemes" | "breakpoints">>>;
type ThemeComponents = Record<string, ComponentStyle | ComponentMultiStyle>;
type BaseTheme = {
    styles?: {
        globalStyle?: UIStyle;
        resetStyle?: UIStyle;
...
type UsageTheme = BaseTheme & {
    [key: string]: any;
};
type ComponentBaseStyle = UIStyle;
type ComponentVariants = Record<string, UIStyle>;
type ComponentSizes = Record<string, UIStyle>;
type ComponentDefaultProps<Y extends Dict = Dict, M extends keyof Theme["components"] | unknown = unknown> = Partial<Omit<Y, "variant" | "size" | "colorScheme">> & {
    variant?: ThemeProps<M>["variant"];
    size?: ThemeProps<M>["size"];
    colorScheme?: ThemeProps<M>["colorScheme"];
};
type ComponentStyle<Y extends Dict = Dict> = {
    /**
...
type ComponentMultiBaseStyle = UIMultiStyle;
type ComponentMultiVariants = Record<string, UIMultiStyle>;
type ComponentMultiSizes = Record<string, UIMultiStyle>;
type ComponentMultiStyle<Y extends Dict = Dict> = {
    /**
     * The base style of the component.
     */
...
type PropsTheme<T extends object = Dict> = T & {
...
type StyledTheme<T extends object = Dict> = T & {
    __config: ThemeConfig;
    __cssVars: Dict;
    __cssMap: CSSMap;
    __breakpoints: AnalyzeBreakpointsReturn;
...
declare const createQuery: (min?: number, max?: number) => string | undefined;
declare const analyzeBreakpoints: (breakpoints: Dict, direction?: BreakpointDirection) => Breakpoints | undefined;
type AnalyzeBreakpointsReturn = ReturnType<typeof analyzeBreakpoints>;
declare const getMinMaxQuery: (queries: BreakpointQueries, direction: BreakpointDirection, pickKey?: string[]) => {
    minQuery: BreakpointQuery;
    maxQuery: BreakpointQuery;
};
...
type ThemeToken = (typeof primaryTokens)[number] | (typeof secondaryTokens)[number] | "animations" | "breakpoints" | "transitions.duration" | "transitions.property" | "transitions.easing";
declare const transformTheme: (theme: Dict, config?: ThemeConfig) => Dict;
declare const omitThemeProps: <T extends ThemeProps, K extends Exclude<keyof T, "colorScheme" | "variant" | "size"> = never>(props: T, keys?: K[]) => Omit<T, "colorScheme" | "variant" | "size" | K>;
type MergeStyleOptions = Omit<Partial<FilterStyleOptions>, "isMulti">;
declare const mergeStyle: (target: ComponentStyle, ...sources: ComponentStyle[]) => ({ omit, pick }?: MergeStyleOptions) => ComponentStyle;
declare const mergeMultiStyle: (target: ComponentMultiStyle, ...sources: ComponentMultiStyle[]) => ({ omit, pick }?: MergeStyleOptions) => ComponentMultiStyle;
...
declare const pickStyle: (target: ComponentMultiStyle, targetKey: string, withProps?: boolean) => ComponentStyle;

declare const createVars: (tokens: VarTokens, prefix?: string) => ({ baseTokens, cssMap, cssVars, }?: {
...
declare const pseudos: {
     ...
     */
    _scrollbarButton: string;
    /**
     * The CSS `::-webkit-scrollbar-corner` pseudo-element.
     */
    _scrollbarCorner: string;
    /**
...
declare const standardStyles: Configs;
declare const shorthandStyles: Configs;
declare const atRuleStyles: Configs;
declare const styles: Configs;
declare const processSkipProperties: string[];
declare const styleProperties: any[];
declare const layoutStyleProperties: any[];
type StyleProps = {
    /**
...
type ThemeProps<Y extends keyof Theme["components"] | unknown = unknown> = {
    variant?: ThemeVariant<Y>;
    size?: ThemeSize<Y>;
    colorScheme?: Theme["colorSchemes"];
};
type ColorModeArray<Y> = [Y, Y];
type ResponsiveObject<Y> = Partial<Record<Theme["breakpoints"] | "base", Y>>;
type UIValue<Y> = ResponsiveObject<Y> | ColorModeArray<Y> | Y;
type BaseToken<Y, M = unknown> = M extends keyof Theme ? Y | Theme[M] : Y;
type ColorModeToken<Y, M = unknown> = M extends keyof Theme ? ColorModeArray<Y | Theme[M]> | Y | Theme[M] : ColorModeArray<Y> | Y;
type ResponsiveToken<Y, M = unknown> = M extends keyof Theme ? ResponsiveObject<Y | Theme[M]> | Y | Theme[M] : ResponsiveObject<Y> | Y;
type Token<Y, M = unknown> = M extends keyof Theme ? UIValue<Y | Theme[M]> : UIValue<Y>;
type StyledProps<Y> = Y | ((theme: StyledTheme) => Y);
type StyleProperties = CSS.Properties & Omit<StyleProps, keyof CSS.Properties>;
type StyleValue<Y extends keyof StyleProperties> = StyledProps<UIValue<StyleProperties[Y]>>;
type StyleUIValue = {
    [Y in keyof StyleProperties]?: Y extends keyof StyleProps ? StyleProps[Y] | StyleValue<Y> : StyleValue<Y>;
};
...
type RecursivePseudos<Y> = {
    [K in keyof CSS.Pseudos | keyof PseudoProps]?: PseudoDefinition<Y> & Y;
};
type RecursiveCSSUIObject<Y> = Y & (Y | RecursivePseudos<Y> | RecursiveStyles<Y>);
type CSSUIObject = RecursiveCSSUIObject<StyleUIValue>;
type CSSUIProps = StyleProps & PseudoProps;
type UIStyleProps = {
    theme: StyledTheme;
    colorMode?: ColorMode;
    colorScheme?: Theme["colorSchemes"];
...
type UIStyle = CSSUIObject | ((props: UIStyleProps) => CSSUIObject);
type UIMultiStyle = Record<string, UIStyle> | ((props: UIStyleProps) => Record<string, UIStyle>);
type AnimationStyle = {
...
declare const css: (cssOrFunc: CSSObjectOrFunc | CSSUIObject) => (theme: StyledTheme, disableStyleProp?: (prop: string) => boolean) => Dict;
...
type CSSProperties = Union<keyof CSS.StandardProperties | keyof CSS.SvgProperties | keyof CSS.ObsoleteProperties>;
type Transform = (value: any, theme: StyledTheme, css: (cssObject: CSSObjectOrFunc | CSSUIObject) => (theme: StyledTheme) => Dict, prev?: Dict) => any;
type ConfigProps = {
    static?: CSSObject;
    isProcessResult?: boolean;
    isProcessSkip?: boolean;
    properties?: CSSProperties | CSSProperties[] | ((theme: StyledTheme) => CSSProperties);
    token?: ThemeToken;
    transform?: Transform;
    /**
...
type Configs = Record<string, ConfigProps | true>;
declare const generateGradient: Transform;
declare const generateAnimation: Transform;
declare const mode: <L extends unknown, D extends unknown>(light: L, dark: D) => (colorMode?: ColorMode | undefined) => L | D;
declare const keyframes: (...arg: CSSObject[]) => Keyframes;
type Transforms = keyof typeof transforms;
declare const transforms: {
    var: (values: any[], theme: StyledTheme) => Dict;
    token: (token: ThemeToken, transform?: Transform, compose?: Transform) => Transform;
    styles: (prefix?: string) => Transform;
    px: (value: any) => any;
    deg: (value: any) => any;
    fraction: (transform?: Transform) => Transform;
    ...
    function: (func: string, transform?: Transform) => Transform;
    gradient: Transform;
    animation: Transform;
    transform: Transform;
    filter: (type?: "filter" | "backdrop") => Transform;
    media: Transform;
    container: Transform;
    supports: Transform;
};
...
type BaseStyle = CSSUIObject | ((props: StyledResolverProps) => CSSUIObject);
type StyledOptions = {
...
type StyledResolverProps = CSSUIObject & {
    theme: PropsTheme;
    /**
...
type OmitProps<Y, M extends keyof any = never> = Omit<Y, "as" | M>;
type IntersectionProps<Y extends object = {}, M extends object = {}> = OmitProps<Y, keyof M> & M;
type PropsOf<T extends As> = React.ComponentPropsWithoutRef<T> & {
    as?: As;
};
type ComponentProps<Y extends object, M extends object, D extends object = {}, H extends As = As> = (IntersectionProps<Y, D> | IntersectionProps<M, D>) & {
    as?: H;
};
type ComponentArgs = {
    displayName?: string;
...
type Component<Y extends As, M extends object = {}> = {
    <D extends As = Y>(props: ComponentProps<React.ComponentProps<Y>, React.ComponentProps<D>, M, D>): JSX.Element;
} & ComponentArgs;
...
type UIComponent<Y extends As, M extends object = {}> = Component<Y, Assign<UIProps, M>>;
type HTMLUIProps<Y extends As> = Omit<PropsOf<Y>, Y extends "svg" ? "ref" | "children" | keyof StyleProps : "ref" | keyof StyleProps> & UIProps & {
    as?: As;
};
type UIPropGetter<Y extends As = "div", M = undefined, D = undefined> = (props?: Merge<HTMLUIProps<Y>, M>, ref?: React.Ref<any>) => Merge<HTMLUIProps<Y> & React.RefAttributes<any>, D>;
type RequiredUIPropGetter<Y extends As = "div", M = undefined, D = undefined> = (props: Merge<HTMLUIProps<Y>, M>, ref?: React.Ref<any>) => Merge<HTMLUIProps<Y> & React.RefAttributes<any>, D>;

declare const forwardRef: <Props extends object, Element extends As, withAs extends boolean = true>(component: React.ForwardRefRenderFunction<any, IntersectionProps<PropsOf<Element>, Props> & (withAs extends true ? {
    as?: As;
...
declare const shouldForwardProp: (disableStyleProp?: (prop: string) => boolean) => (prop: string) => boolean;

type ToCSSObject = {
    (options: Pick<StyledOptions, "baseStyle" | "disableStyleProp">): FunctionInterpolation<StyledResolverProps>;
};
declare const toCSSObject: ToCSSObject;
declare const styled: <T extends As, P extends object = {}>(element: T, { baseStyle, disableStyleProp, ...styledOptions }?: StyledOptions) => UIComponent<T, P>;
 ...
 */
declare const ui: UIFactory & HTMLUIComponents;

type ThemeSchemeManager = {
...
declare const createThemeSchemeManager: (type?: "local" | "cookie" | "ssr", cookie?: string) => ThemeSchemeManager;
...
type ThemeProviderProps = Omit<ThemeProviderProps$1, "theme"> & ThemeProviderOptions;
declare const ThemeProvider: FC<ThemeProviderProps>;
declare const CSSVars: FC;
declare const ResetStyle: FC;
 ...
 */
declare const useTheme: <T extends object = Dict>() => Pick<PropsTheme, "themeScheme" | "changeThemeScheme"> & {
    theme: StyledTheme<T>;
...
declare const getThemeSchemeScript: ({ initialThemeScheme: init, type, storageKey, }?: Omit<ThemeSchemeScriptProps, "nonce">) => string;
declare const ThemeSchemeScript: ({ nonce, ...rest }: ThemeSchemeScriptProps) => react_jsx_runtime.JSX.Element;

type ColorModeManager = {
    type: "cookie" | "localStorage";
    ssr?: boolean;
    get: (initColorMode?: ColorMode | "system") => (storageKey?: string) => ColorMode | "system";
    set: (colorMode: ColorMode | "system") => (storageKey?: string) => void;
};
declare const createColorModeManager: (type?: "local" | "cookie" | "ssr", cookie?: string) => ColorModeManager;
...
declare const ColorModeProvider: FC<ColorModeProviderProps>;
/**
 ...
 */
declare const useColorModeValue: <L extends unknown, D extends unknown>(light: L, dark: D) => L | D;
declare const getColorModeValue: <L extends unknown, D extends unknown>(light: L, dark: D) => (colorMode: ColorMode) => L | D;
...
declare const getColorModeScript: ({ initialColorMode, type, storageKey, }?: Omit<ColorModeScriptProps, "nonce">) => string;
declare const ColorModeScript: ({ nonce, ...rest }: ColorModeScriptProps) => react_jsx_runtime.JSX.Element;

export { type AlertStatus, type AlertStatuses, type AnalyzeBreakpointsReturn, type AnimationStyle, type As, type BaseStyle, type BaseToken, type BreakpointDirection, type BreakpointQueries, COLOR_MODE_STORAGE_KEY, type CSSMap, type CSSObjectOrFunc, type CSSUIObject, type CSSUIProps, CSSVars, type ChangeThemeScheme, type ColorMode, type ColorModeArray, ColorModeContext, type ColorModeManager, ColorModeProvider, type ColorModeProviderProps, ColorModeScript, type ColorModeScriptProps, type ColorModeToken, type Component, type ComponentArgs, type ComponentBaseStyle, type ComponentDefaultProps, type ComponentMultiBaseStyle, type ComponentMultiSizes, type ComponentMultiStyle, type ComponentMultiVariants, type ComponentProps, type ComponentSizes, type ComponentStyle, type ComponentVariants, type ConfigProps, type Configs, type CustomTheme, type DOMElements, type FunctionCSSInterpolation, GeneratedTheme, GlobalStyle, type HTMLUIComponents, type HTMLUIProps, type IntersectionProps, type LayerStyles, type LoadingComponentProps, type LoadingConfigOptions, type LoadingVariant, type NoticeComponentProps, type NoticeConfigOptions, type NoticePlacement, type OmitProps, type PropsOf, type PropsTheme, type RecursiveCSSUIObject, type RecursivePseudos, type RecursiveStyles, type RequiredUIPropGetter, ResetStyle, type ResponsiveObject, type ResponsiveToken, type SnackComponentProps, type SnackConfigOptions, type SnackDirection, type StyleProperties, type StyleProps, type StyleUIValue, type StyledOptions, type StyledProps, type StyledResolverProps, type StyledTheme, THEME_SCHEME_STORAGE_KEY, type TextStyles, type Theme, type ThemeAnimationTokens, type ThemeBreakpointTokens, type ThemeComponents, type ThemeConfig, type ThemeProps, ThemeProvider, type ThemeProviderProps, type ThemeSchemeManager, ThemeSchemeScript, type ThemeSchemeScriptProps, type ThemeSchemes, type ThemeSemantics, type ThemeToken, type ThemeTokens, type ThemeTransitionTokens, type Token, type Transform, type Transforms, type UIComponent, type UIFactory, type UIMultiStyle, type UIPropGetter, type UIProps, type UIStyle, type UIStyleProps, UITheme, type UsageTheme, type VarTokens, analyzeBreakpoints, atRuleStyles, colorModeManager, createColorModeManager, createQuery, createThemeSchemeManager, createVars, css, forwardRef, generateAnimation, generateGradient, getCSS, getColorModeScript, getColorModeValue, getMinMaxQuery, getThemeSchemeScript, keyframes, layoutStyleProperties, mergeMultiStyle, mergeStyle, mode, omitThemeProps, pickStyle, processSkipProperties, shorthandStyles, shouldForwardProp, standardStyles, styleProperties, styled, styles, themeSchemeManager, toCSSObject, transformTheme, transforms, ui, useCSS, useColorMode, useColorModeValue, useComponentStyle, useMultiComponentStyle, useTheme };
