...
var styleSheet = (0, import_utils.createdDom)() ? new import_sheet.StyleSheet({ key: "css", container: document.head }) : void 0;
var directions = {
  "to-t": "to top",
  "to-tr": "to top right",
  "to-r": "to right",
...
var isCSSFunction = (value) => (0, import_utils.isString)(value) && value.includes("(") && value.includes(")");
...
var generateGradient = (value, theme) => {
  ...
  const values = colors.map((_color) => {
    ...
    if (ratio) {
      return [color, ...(0, import_utils.isArray)(ratio) ? ratio : [ratio]].join(" ");
    } else {
      return color;
    }
  });
...
var generateAnimation = (value, theme, css2) => {
  ...
  if ((0, import_utils.isObject)(value)) {
    ...
    const { name, styles: styles2 } = (0, import_react.keyframes)(keyframes2);
...
var generateAtRule = (identifier) => (values, theme) => values.reduce(
  (prev, {
    ...
    if (!query) {
      const resolvedRest = (0, import_utils.filterUndefined)({
        width,
        minWidth,
        maxWidth,
        height,
      ...
      query = Object.entries(resolvedRest).map(([key, value]) => {
        ...
        return `(${(0, import_utils.toKebabCase)(key)}: ${value})`;
      }).join(" and ");
    }
...
var keyframes = (...arg) => (0, import_react.keyframes)(...arg);
var transforms = {
  ...
  styles: (prefix) => (value, theme, _css, prev = {}) => {
    const resolvedCSS = {};
    const style = (0, import_utils.getMemoizedObject)(
      theme,
  ...
  px: (value) => {
    ...
    return isUnitless || (0, import_utils.isNumber)(value) ? `${value}px` : value;
  },
  deg: (value) => {
    if (isCSSVar(value) || value == null)
      return value;
    const isUnitless = typeof value === "string" && !value.endsWith("deg");
    return isUnitless || (0, import_utils.isNumber)(value) ? `${value}deg` : value;
  },
  fraction: (transform) => (value, ...rest) => {
    if ((0, import_utils.isNumber)(value) && value <= 1) {
      value = `${value * 100}%`;
    }
    if (transform)
...
var transformBreakpoints = (breakpoints, direction) => {
  return Object.fromEntries(
    Object.entries(breakpoints).map(([name, value]) => [name, (0, import_utils2.getPx)(value)]).sort((a, b) => {
      if (direction === "down") {
        return b[1] - a[1];
      } else {
        return a[1] - b[1];
...
var analyzeBreakpoints = (breakpoints, direction = "down") => {
  ...
  const keys = (0, import_utils2.keysFormObject)(breakpoints);
...
var expandCSS = (css2, isNested) => (theme) => {
  ...
  for (let [key, value] of Object.entries(css2)) {
    value = (0, import_utils3.runIfFunc)(value, theme);
    if (value == null)
      continue;
    if ((0, import_utils3.isArray)(value) && !(isProcessSkip(key) && !isNested)) {
      computedCSS = (0, import_utils3.merge)(computedCSS, expandColorMode(key, value));
      continue;
    }
    if ((0, import_utils3.isObject)(value) && isResponsive(value) && !(isProcessSkip(key) && !isNested)) {
      computedCSS = (0, import_utils3.merge)(computedCSS, expandResponsive(key, value, queries2));
      continue;
    }
...
var getCSS = ({
  ...
  const createCSS = (cssOrFunc, isNested = false) => {
    var _a, _b, _c;
    const cssObject = (0, import_utils3.runIfFunc)(cssOrFunc, theme);
    ...
    for (let [prop, value] of Object.entries(computedCSS)) {
      ...
      if ((0, import_utils3.isObject)(value) && !(style == null ? void 0 : style.isProcessSkip)) {
        resolvedCSS[prop] = (_a = resolvedCSS[prop]) != null ? _a : {};
        resolvedCSS[prop] = (0, import_utils3.merge)(resolvedCSS[prop], createCSS(value, true));
        continue;
      }
      ...
      if (!isNested && (style == null ? void 0 : style.static)) {
        const staticStyles = (0, import_utils3.runIfFunc)(style.static, theme);
        resolvedCSS = (0, import_utils3.merge)(resolvedCSS, staticStyles);
      }
      const properties = (0, import_utils3.runIfFunc)(style == null ? void 0 : style.properties, theme);
      if (properties) {
        if ((0, import_utils3.isArray)(properties)) {
          for (const property of properties) {
            resolvedCSS[property] = value;
          }
          continue;
        } else if ((0, import_utils3.isObject)(value)) {
          resolvedCSS = (0, import_utils3.merge)(resolvedCSS, value);
          continue;
        } else {
      ...
      if ((0, import_utils3.isObject)(value)) {
        resolvedCSS = (0, import_utils3.merge)(resolvedCSS, value);
        continue;
      }
...
var createVar = (token, prefix) => {
  const variable = `--${[prefix, (0, import_utils4.escape)(token, "-")].filter(Boolean).join("-")}`;
...
var createVars = (tokens, prefix = "ui") => ({
  ...
  for (let [token, { isSemantic, value }] of Object.entries(tokens)) {
    ...
    if (token.startsWith("animations.")) {
      if ((0, import_utils4.isArray)(value)) {
      ...
      let [lightValue, darkValue] = (0, import_utils4.isArray)(value) ? [...value] : [value];
    ...
    if (!isSemantic) {
      if (token.startsWith("spaces") && !(0, import_utils4.isUndefined)(resolvedLightValue)) {
        const keys = token.split(".");
        const [firstKey, ...restKeys] = keys;
        const negativeToken = `${firstKey}.-${restKeys.join(".")}`;
        const negativeValue = import_utils4.calc.negate(resolvedLightValue);
        const negativeReference = import_utils4.calc.negate(reference);
      ...
      if (resolvedDarkValue)
        cssVars = (0, import_utils4.merge)(cssVars, {
    ...
    cssVars = (0, import_utils4.merge)(cssVars, { [variable]: parentRef });
    if (resolvedDarkValue) {
      const [, parentRef2] = fetchParent(resolvedDarkValue);
      cssVars = (0, import_utils4.merge)(cssVars, { [pseudos._dark]: { [variable]: parentRef2 } });
    }
    cssMap[token] = {
...
var useCSS = (cssObject = {}) => {
  ...
  return (0, import_react3.useMemo)(() => getCSS2(cssObject)(theme), [cssObject, theme]);
};
var getCSS2 = (cssObject = {}) => (theme) => (0, import_css2.css)(css(cssObject)(theme));
...
var createTokens = (theme, target = "primary") => {
  ...
  const defaultTokenMap = (0, import_utils5.pickObject)(theme, defaultTokens);
  const semanticTokenMap = (0, import_utils5.pickObject)((_a = theme.semantics) != null ? _a : {}, semanticTokens);
  const defaultTokenEntries = Object.entries(
    (0, import_utils5.flattenObject)(defaultTokenMap, Infinity, omitKeys)
  ).map(([token, value]) => {
    const enhancedToken = { isSemantic: false, value };
    return [token, enhancedToken];
  });
  const semanticTokenEntries = Object.entries(
    (0, import_utils5.flattenObject)(semanticTokenMap, Infinity, omitKeys)
  ).reduce(
    (prev, [token, value]) => {
      if (token.startsWith("colorSchemes.")) {
        const [, semanticToken] = token.split(".");
        import_utils5.tones.forEach((tone) => {
  ...
  return (0, import_utils5.objectFromEntries)([
    ...defaultTokenEntries,
...
var omitTheme = (theme) => (0, import_utils5.omitObject)(theme, ["__cssMap", "__cssVar", "__breakpoints"]);
var omitThemeProps = (props, keys = []) => (0, import_utils5.omitObject)(props, ["size", "variant", "colorScheme", ...keys]);
  ...
  if ((0, import_utils5.isObject)(source) && (0, import_utils5.isObject)(target)) {
    for (const [sourceKey, sourceValue] of Object.entries(source)) {
      ...
      if (target.hasOwnProperty(sourceKey)) {
        if (!(0, import_utils5.isFunction)(targetValue) && !(0, import_utils5.isFunction)(sourceValue)) {
          ...
          result[sourceKey] = (props) => recursiveMergeStyle(
            (0, import_utils5.runIfFunc)(targetValue, props),
            (0, import_utils5.runIfFunc)(sourceValue, props)
          );
        }
      } else {
...
var filterStyle = (target) => ({ omit, pick, isMulti = false }) => {
  if (!(0, import_utils5.isObject)(target))
    return target;
  if (omit.length)
    target = internalFilterStyle(target, omit, isMulti)(import_utils5.omitObject);
  if (pick.length)
    target = internalFilterStyle(target, pick, isMulti)(import_utils5.pickObject);
  return target;
};
var internalFilterStyle = (target, keys, isMulti, refs = []) => (func) => {
  ...
  Object.entries(target != null ? target : {}).forEach(([nestedKey, value]) => {
    ...
    if (!onValidFilterStyleKey(newRefs, isMulti))
      return;
    if ((0, import_utils5.isFunction)(value)) {
      target[nestedKey] = (props) => internalFilterStyle(value(props), newKeys, isMulti, newRefs)(func);
    } else {
      if (func === import_utils5.omitObject || Object.keys(value).some((key) => newKeys.includes(key))) {
        ...
        target[nestedKey] = (0, import_utils5.merge)(
          target[nestedKey],
    ...
    switch (key) {
      ...
      case "sizes":
        result[key] = Object.entries(
          ...
          if ((0, import_utils5.isFunction)(value2)) {
...
var ColorModeProvider = ({
  ...
  const [colorMode, setColorMode] = (0, import_react4.useState)(
    () => getColorMode(colorModeManager2, initialColorMode)(storageKey)
  );
  const [systemColorMode, setSystemColorMode] = (0, import_react4.useState)(
    void 0
  );
  ...
  const { getSystemColorMode, setClassName, setDataset, addListener } = (0, import_react4.useMemo)(
    () => getColorModeUtils({ isPreventTransition: disableTransitionOnChange }),
    [disableTransitionOnChange]
  );
  const changeColorMode = (0, import_react4.useCallback)(
    (colorMode2) => {
  ...
  const changeSystemColorMode = (0, import_react4.useCallback)(
    (systemColorMode2) => {
  ...
  (0, import_utils6.useSafeLayoutEffect)(() => {
  ...
  const context = (0, import_react4.useMemo)(
    () => ({
      ...
      changeColorMode: defaultColorMode ? import_utils6.noop : changeColorMode,
      toggleColorMode: defaultColorMode ? import_utils6.noop : toggleColorMode,
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(ColorModeContext.Provider, { value: context, children });
};
var useColorMode = () => {
...
var ThemeProvider = ({
  ...
  const [themeScheme, setThemeScheme] = (0, import_react6.useState)(
    themeSchemeManager2.get(config == null ? void 0 : config.initialThemeScheme)(storageKey)
  );
  const changeThemeScheme = (0, import_react6.useCallback)(
    (themeScheme2) => {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(import_react5.ThemeProvider, { theme: { themeScheme, changeThemeScheme, ...theme }, children: [
    /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(CSSVars, {}),
    children
  ] });
};
var CSSVars = () => {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(
    import_react5.Global,
    {
      styles: (theme) => {
        ...
        let style = (0, import_utils7.getMemoizedObject)(theme, "styles.resetStyle", {});
        const computedStyle = (0, import_utils7.runIfFunc)(style, {
          theme,
          colorMode,
          themeScheme
        });
...
var GlobalStyle = () => {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(
    import_react5.Global,
    {
      styles: (theme) => {
        const { themeScheme } = theme;
        let style = (0, import_utils7.getMemoizedObject)(theme, "styles.globalStyle", {});
        const computedStyle = (0, import_utils7.runIfFunc)(style, {
          theme,
          colorMode,
          themeScheme
...
var useTheme = () => {
  const { themeScheme, changeThemeScheme, ...internalTheme } = (0, import_react6.useContext)(
    import_react5.ThemeContext
  );
  const theme = (0, import_react6.useMemo)(() => {
    var _a;
    if ((0, import_utils7.isUndefined)(themeScheme) || themeScheme === "base")
      return internalTheme;
    const nestedTheme = (_a = internalTheme.themeSchemes) == null ? void 0 : _a[themeScheme];
    if (!nestedTheme)
      return internalTheme;
    return (0, import_utils7.merge)(internalTheme, nestedTheme);
...
var getResponsiveStyles = (value, modifierStyles, props) => ({ isMulti }) => {
  var _a, _b;
  const providedKeys = (0, import_utils8.keysFormObject)(value);
    ...
    return queries2.reduce(
      (prev, { breakpoint: breakpoint2, minW, maxW, maxWQuery, minWQuery }, index) => {
        ...
        if (!hasBaseStyles) {
          ...
          prev = (0, import_utils8.merge)(prev, baseStyles);
          hasBaseStyles = true;
        }
        ...
        prev = (0, import_utils8.merge)(prev, queryStyles);
        return prev;
      },
      {}
    );
...
var getModifierStyles = (value, modifierStyles, props) => ({ isMulti }) => {
  ...
  if ((0, import_utils8.isArray)(value)) {
    ...
    styles2 = (0, import_utils8.merge)(lightStyles, darkStyles);
...
var getStyles = (stylesOrFunc, props) => ({ isMulti, query }) => {
  let styles2 = (0, import_utils8.runIfFunc)(stylesOrFunc, props);
  if (isMulti) {
    for (const [key, styleOrFunc] of Object.entries(
      styles2 != null ? styles2 : {}
    )) {
      const style = (0, import_utils8.runIfFunc)(styleOrFunc, props);
      if (query) {
        styles2 = (0, import_utils8.merge)(styles2, { [key]: { [query]: style } });
      } else {
        styles2 = (0, import_utils8.merge)(styles2, { [key]: style });
      }
    }
  } else if (query) {
...
var setStyles = (name, props, isMulti = false) => {
  ...
  const { colorMode } = useColorMode();
  const componentStyle = (0, import_utils8.getMemoizedObject)(
    theme,
    `components.${name}`
  );
  const propsRef = (0, import_react7.useRef)({});
  const stylesRef = (0, import_react7.useRef)({});
  props = (0, import_utils8.merge)((_a = componentStyle == null ? void 0 : componentStyle.defaultProps) != null ? _a : {}, (0, import_utils8.filterUndefined)(props));
  if (componentStyle) {
    const args = (0, import_utils8.omitObject)(props, ["children"]);
    ...
    styles2 = (0, import_utils8.merge)(styles2, sizeStyles);
    styles2 = (0, import_utils8.merge)(styles2, variantStyles);
    const isStylesEqual = (0, import_react_fast_compare.default)(stylesRef.current, styles2);
    if (!isStylesEqual)
...
var emotionStyled = (0, import_utils9.interopDefault)(import_styled.default);
...
var toCSSObject = ({ baseStyle, disableStyleProp }) => (props) => {
  ...
  const propsCSS = (0, import_utils9.filterObject)(
    rest,
    (prop) => prop in styleProps
  );
  const baseCSS = (0, import_utils9.runIfFunc)(baseStyle, props);
  const computedCSS = css(
    (0, import_utils9.assignAfter)({}, __css, baseCSS, (0, import_utils9.filterUndefined)(propsCSS), sx)
...
var styled = (element, { baseStyle, disableStyleProp, ...styledOptions } = {}) => {
  ...
  const UIComponent = (0, import_react8.forwardRef)((props, ref) => {
    const { colorMode, forced } = useColorMode();
    return (0, import_react8.createElement)(Component, {
      ref,
